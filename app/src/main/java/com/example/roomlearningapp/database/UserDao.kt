...
interface UserDao {
    ...
    @Query("UPDATE user_table SET color_option=:color WHERE id = :id")
    suspend fun updateColor(color: Int, id: Int?)

    @Query("UPDATE user_table SET first_name =:firstName ,last_name=:lastName, color_option=:color WHERE id = :id")
